...
// Helper function to create an API manager with default configurations
pub async fn create_api_manager(configs: HashMap<String, APIConfig>) -> APIManager {
    let manager = APIManager::new();
    manager.initialize_default_clients(configs).await;
    manager
}